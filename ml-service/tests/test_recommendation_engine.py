...
from app.data.career_dataset import DATASET_CAREER_COUNT
...
class TestScoreBounds:
    def test_scores_always_within_0_100(self, predictor, career_db):
        ...
        for profile in profiles:
            results = predictor.predict_career_hybrid(profile, {}, top_k=DATASET_CAREER_COUNT)
            for r in results:
...
class TestEvidenceEffects:
    def test_verified_skills_never_lower_the_score(self, predictor):
        ...
        unverified = get(predictor.predict_career_hybrid(profile, {}, top_k=DATASET_CAREER_COUNT), 'Data Scientist')
        verified = get(
            predictor.predict_career_hybrid(profile, {'python': True, 'sql': True}, top_k=DATASET_CAREER_COUNT), 'Data Scientist'
        )
    ...
    def test_matching_education_increases_score_over_no_education(self, predictor, career_db):
        ...
        r_with = get(predictor.predict_career_hybrid(with_edu, {}, top_k=DATASET_CAREER_COUNT), 'Chartered Accountant')
        r_without = get(predictor.predict_career_hybrid(without_edu, {}, top_k=DATASET_CAREER_COUNT), 'Chartered Accountant')
    ...
    def test_relevant_project_content_increases_score_over_blank_project(self, predictor, career_db):
        ...
        r_with = get(predictor.predict_career_hybrid(with_project, {}, top_k=DATASET_CAREER_COUNT), 'Web Developer')
        r_without = get(predictor.predict_career_hybrid(without_project, {}, top_k=DATASET_CAREER_COUNT), 'Web Developer')
        assert r_with['fit_score'] > r_without['fit_score']
    ...
    def test_experience_increases_score_monotonically(self, predictor, career_db):
        ...
        for years in [0, 2, 5, 8]:
            ...
            r = get(predictor.predict_career_hybrid(profile, {}, top_k=DATASET_CAREER_COUNT), 'Business Analyst')
...
class TestExplainability:
    def test_reasoning_cites_actual_matched_and_missing_skills(self, predictor, career_db):
        ...
        r = get(predictor.predict_career_hybrid(profile, {}, top_k=DATASET_CAREER_COUNT), 'Data Scientist')
